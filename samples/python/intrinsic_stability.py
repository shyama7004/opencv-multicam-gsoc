...
def load_data_from_yaml(yaml_path):
    """Load 2D image points, 3D pattern points, and image sizes from an OpenCV YAML file."""
    ...
    # reshape flattened block if needed
...
def sim_intrinsic_stability(img_pts, obj_pts, img_size, n_runs, subset_ratio, jobs, seed):
    random.seed(seed)
...
def analyze_and_plot(yaml_path, n_runs, subset_ratio, jobs, out_dir, seed):
    ...
    for cam_idx in range(num_cams):
        Ks, Dists, Errs = sim_intrinsic_stability(
            ...
            pattern_points,
            image_sizes[cam_idx],
            n_runs, subset_ratio, jobs, seed
        )

        fx, fy = Ks[:,0,0], Ks[:,1,1]
        cx, cy = Ks[:,0,2], Ks[:,1,2]
        # distortion coeffs: assume first 4 elements k1,k2,p1,p2
        k1, k2, p1, p2 = Dists[:,:4].T
        ...
        # boxplot of focal & principal point
        ...
        # histogram of RMS reprojection error
...
def parse_args():
    ...
    p.add_argument('--jobs',   type=int,   default=1,   help="Parallel jobs (-1 for all cores)")
